...
class Person:
    ...
    def parse(personInfo: str):
        ...
        pattern = "\\[" + float_pattern + "]" + int_pattern + "-PRI-" + int_pattern + "-FROM-" + floor_pattern + "-TO-" + floor_pattern + "-BY-" + int_pattern
        ...
        targetElevator = int(match.group(6))
        ...
        if (not (1 <= targetElevator and targetElevator <= 6)):
            raise Exception(f"Invalid index of targetElevator info: {personInfo}")
